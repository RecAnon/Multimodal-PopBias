...
class SOIL(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        image_interest_file = os.path.join(
            dataset_path, "image_interest_{}_{}.pt".format(self.knn_k, self.knn_i)
        )
        text_interest_file = os.path.join(
            dataset_path, "text_interest_{}_{}.pt".format(self.knn_k, self.knn_i)
        )

        mm_attractive_file = os.path.join(
            dataset_path, "mm_attractive_{}.pt".format(self.knn_a)
        )

        self.norm_adj = self.get_adj_mat()
        self.R = self.sparse_mx_to_torch_sparse_tensor(self.R).float().to(self.device)
        self.norm_adj = (
            self.sparse_mx_to_torch_sparse_tensor(self.norm_adj).float().to(self.device)
        )

        if self.v_feat is not None:
            self.image_embedding = nn.Embedding.from_pretrained(
                self.v_feat, freeze=True
            )
        ...
        if os.path.exists(image_interest_file) and os.path.exists(text_interest_file):
            ...
            image_interest_adj = build_non_zero_graph(
                image_interest, is_sparse=self.sparse, norm_type="sym"
            )
            text_interest_adj = build_non_zero_graph(
                text_interest, is_sparse=self.sparse, norm_type="sym"
            )
        ...
        # Second-Order graph
        if os.path.exists(mm_attractive_file) and os.path.exists(
            mm_attractive_file.replace(".pt", "_R.pt")
        ):
            ...
            for user, items in dataset.history_items_per_u.items():
                ...
                mm_sparse = torch.sparse_coo_tensor(
                    torch.stack([uid, self.n_users + mm_indices]),
                    k_mm_value,
                    size=self.norm_adj.size(),
                )
                mm_sparse_t = torch.sparse_coo_tensor(
                    torch.stack([self.n_users + mm_indices, uid]),
                    k_mm_value,
                    size=self.norm_adj.size(),
                )

                mm_R_sparse = torch.sparse_coo_tensor(
                    torch.stack([uid, mm_indices]), k_mm_value, size=self.R.size()
                )
            ...
            mm_attractive_adj = build_graph_from_adj(
                mm_attractive, is_sparse=self.sparse, norm_type="sym", mask=False
            )
            mm_attractive_adj_R = build_graph_from_adj(
                mm_attractive_R, is_sparse=self.sparse, norm_type="sym", mask=False
            )
        ...
        self.map_v = nn.Sequential(
            nn.Linear(self.embedding_dim, self.embedding_dim), nn.Sigmoid()
        )
    ...
    def forward(self, adj, train=False):
        ...
        image_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.map_v(image_feats)
        )
        text_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.map_t(text_feats)
        )
        ...
        for i in range(self.n_layers):
            image_item_embeds = torch.sparse.mm(
                self.image_interest_adj, image_item_embeds
            )
        ...
        att_common = torch.cat(
            [self.attention(image_embeds), self.attention(text_embeds)], dim=-1
        )
        weight_common = self.softmax(att_common)
        common_embeds = (
            weight_common[:, 0].unsqueeze(dim=1) * image_embeds
        ...
        all_embeddings_users, all_embeddings_items = torch.split(
    ...
    def calculate_loss(self, interaction):
        ...
        cl_loss = self.InfoNCE(
            side_embeds_items[pos_items], content_embeds_items[pos_items], 0.2
        ) + self.InfoNCE(side_embeds_users[users], content_embeds_user[users], 0.2)
        # user-item constractive loss
        cl_loss2 = self.InfoNCE(
            u_g_embeddings, content_embeds_items[pos_items], 0.2
        ) + self.InfoNCE(u_g_embeddings, side_embeds_items[pos_items], 0.2)
