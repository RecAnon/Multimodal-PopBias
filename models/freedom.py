...
class FREEDOM(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        if self.t_feat is not None:
            ...
            text_feats = self.text_trs(self.t_feat)
            mf_t_loss = self.bpr_loss(
                ua_embeddings[users], text_feats[pos_items], text_feats[neg_items]
            )
        if self.v_feat is not None:
            # image_feats = self.image_trs(self.image_embedding.weight)
            image_feats = self.image_trs(self.v_feat)
            mf_v_loss = self.bpr_loss(
                ua_embeddings[users], image_feats[pos_items], image_feats[neg_items]
            )
