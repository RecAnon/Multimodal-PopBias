...
class GUME(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.image_space_trans = nn.Sequential(
            self.image_reduce_dim, self.image_trans_dim
        )
        ...
        self.text_space_trans = nn.Sequential(self.text_reduce_dim, self.text_trans_dim)
    ...
    def add_edge(self, inter):
        ...
        item_adj = sp.coo_matrix(
            (sim_values, (sim_rows, sim_cols)),
    ...
    def bpr_loss(self, users, pos_items, neg_items):
        ...
        regularizer = (
            self.sq_sum(users) + self.sq_sum(pos_items) + self.sq_sum(neg_items)
        ) / self.batch_size
    ...
    def align_vt(self, embed1, embed2):
        ...
        vt_loss = (
            torch.abs(emb1_var - emb2_var) + torch.abs(emb1_mean - emb2_mean)
        ).mean()
    ...
    def forward_pred(self):
        return [x.cpu() for x in self.forward(self.norm_adj)]
