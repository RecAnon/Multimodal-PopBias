...
class MGCN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        image_adj_file = os.path.join(
            dataset_path, "image_adj_{}_{}.pt".format(self.knn_k, self.sparse)
        )
        text_adj_file = os.path.join(
            dataset_path, "text_adj_{}_{}.pt".format(self.knn_k, self.sparse)
        )
    ...
    def forward(self, adj, train=False):
        ...
        image_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.gate_v(image_feats)
        )
        text_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.gate_t(text_feats)
        )
        ...
        if self.sparse:
            for i in range(self.n_layers):
                image_item_embeds = torch.sparse.mm(
                    self.image_original_adj, image_item_embeds
                )
        else:
            for i in range(self.n_layers):
        ...
        if self.sparse:
            for i in range(self.n_layers):
                text_item_embeds = torch.sparse.mm(
                    self.text_original_adj, text_item_embeds
                )
        else:
            for i in range(self.n_layers):
        ...
        att_common = torch.cat(
            [self.query_common(image_embeds), self.query_common(text_embeds)], dim=-1
        )
